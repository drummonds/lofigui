...
const appVersion = "Water Tank HTMX v1.0"
...
// setupRoutes registers all HTTP handlers for the water tank HTMX app.
// The returned mux works identically in both server and WASM service worker modes.
func setupRoutes(sim *Simulation) *http.ServeMux {
	...
	mux.HandleFunc("GET /fragment/diagnostics", func(w http.ResponseWriter, r *http.Request) {
		content := renderAndCapture(func() { renderDiagnostics(sim) })
		w.Header().Set("Content-Type", "text/html; charset=utf-8")
		fmt.Fprint(w, content)
	})
	...
	mux.HandleFunc("POST /pump", func(w http.ResponseWriter, r *http.Request) {
		sim.TogglePump()
		http.Redirect(w, r, "/", http.StatusSeeOther)
	})

	// POST /valve — toggle valve
	mux.HandleFunc("POST /valve", func(w http.ResponseWriter, r *http.Request) {
		sim.ToggleValve()
		http.Redirect(w, r, "/", http.StatusSeeOther)
	})
