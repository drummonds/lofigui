...
// sparklineSVG renders a sparkline with filled area as inline SVG.
func sparklineSVG(values []float64, title string) string {
	w, h := 400, 80
	padTop := 25
	n := len(values)
	if n < 2 {
	...
	for i, v := range values {
		x := 10 + float64(i)*float64(w-20)/float64(n-1)
		y := float64(padTop) + 5 + float64(h-10)*(1-v/maxVal)
		points[i] = fmt.Sprintf("%.1f,%.1f", x, y)
	}

	// Fill area: line points + bottom-right + bottom-left
	fillPoints := make([]string, n+2)
	copy(fillPoints, points)
	fillPoints[n] = fmt.Sprintf("%.1f,%.1f", 10+float64(w-20), float64(padTop+h))
	fillPoints[n+1] = fmt.Sprintf("%.1f,%.1f", 10.0, float64(padTop+h))

	var sb strings.Builder
	fmt.Fprintf(&sb, `<svg width="%d" height="%d" xmlns="http://www.w3.org/2000/svg">`, w, padTop+h+10)
	fmt.Fprintf(&sb, `<text x="%d" y="16" text-anchor="middle" font-size="13" font-weight="bold" fill="#333">%s</text>`,
		w/2, title)
	fmt.Fprintf(&sb, `<polygon points="%s" fill="#3298dc" fill-opacity="0.15"/>`,
		strings.Join(fillPoints, " "))
	fmt.Fprintf(&sb, `<polyline points="%s" fill="none" stroke="#3298dc" stroke-width="2"/>`,
		strings.Join(points, " "))

	// Dots
	for _, p := range points {
		parts := strings.Split(p, ",")
		fmt.Fprintf(&sb, `<circle cx="%s" cy="%s" r="3" fill="#3298dc"/>`, parts[0], parts[1])
	}
