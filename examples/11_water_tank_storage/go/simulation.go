...
// SimState is the serializable simulation state for persistence.
type SimState struct {
	TankLevel   float64 `json:"tank_level"`
	PumpOn      bool    `json:"pump_on"`
	ValveOpen   bool    `json:"valve_open"`
	PumpCycles  int     `json:"pump_cycles"`
	PumpOnTime  string  `json:"pump_on_time"`
	ValveCycles int     `json:"valve_cycles"`
	ValveOnTime string  `json:"valve_on_time"`
	FloatTrips  int     `json:"float_trips"`
	TickCount   int     `json:"tick_count"`
}

// LogEntry is a persistent log record.
...
// DiagRecord is a persistent diagnostic snapshot.
...
// ExportState returns a serializable snapshot of the simulation state.
func (s *Simulation) ExportState() SimState {
	s.mu.Lock()
	defer s.mu.Unlock()
	...
	if s.pumpOn && !s.pumpLastOn.IsZero() {
		pumpOn += now.Sub(s.pumpLastOn)
	}
	valveOn := s.valveOnTime
	if s.valveOpen && !s.valveLastOpen.IsZero() {
		valveOn += now.Sub(s.valveLastOpen)
	}
	...
	return SimState{
		...
		PumpOnTime:  pumpOn.Truncate(time.Second).String(),
		ValveCycles: s.valveCycles,
		ValveOnTime: valveOn.Truncate(time.Second).String(),
		FloatTrips:  s.floatTrips,
		TickCount:   s.tickCount,
	}
}

// ImportState restores simulation state from a serialized snapshot.
func (s *Simulation) ImportState(st SimState) {
	s.mu.Lock()
	defer s.mu.Unlock()

	s.tankLevel = st.TankLevel
	s.pumpOn = st.PumpOn
	s.valveOpen = st.ValveOpen
	s.pumpCycles = st.PumpCycles
	s.valveCycles = st.ValveCycles
	s.floatTrips = st.FloatTrips
	s.tickCount = st.TickCount

	if d, err := time.ParseDuration(st.PumpOnTime); err == nil {
		s.pumpOnTime = d
	}
	if d, err := time.ParseDuration(st.ValveOnTime); err == nil {
		s.valveOnTime = d
	}
}

// ExportDiagRecord returns a DiagRecord for persistence.
func (s *Simulation) ExportDiagRecord() DiagRecord {
	st := s.ExportState()
	return DiagRecord{
		Timestamp:   time.Now().UTC().Format(time.RFC3339),
		TankLevel:   st.TankLevel,
		PumpCycles:  st.PumpCycles,
		PumpOnTime:  st.PumpOnTime,
		ValveCycles: st.ValveCycles,
		ValveOnTime: st.ValveOnTime,
		FloatTrips:  st.FloatTrips,
