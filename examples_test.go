...
func TestGoExamples(t *testing.T) {
	...
	// 01's and 02's templates moved into the library (autogenerated).
	// 01a is the first example with an on-disk templates/ dir.
	t.Run("Example01aTemplates", testGoExample01aTemplates)
...
func testGoExample01aTemplates(t *testing.T) {
	examplePath := "examples/01a_hello_world_explicit/go"
...
func testGoExampleModules(t *testing.T) {
	examples := []struct {
	...
	}{
		{
			name:       "01_hello_world",
			path:       "examples/01_hello_world/go",
			moduleName: "git.bytestone.uk/hum3/lofigui/examples/01_hello_world",
		},
		{
			name:       "02_svg_graph",
			path:       "examples/02_svg_graph/go",
			moduleName: "git.bytestone.uk/hum3/lofigui/examples/02_svg_graph",
		},
		{
			name:       "03_style_sampler",
			path:       "examples/03_style_sampler/go",
			moduleName: "git.bytestone.uk/hum3/lofigui/examples/03_style_sampler",
		},
		{
			name:       "06_notes_crud",
			path:       "examples/06_notes_crud/go",
			moduleName: "git.bytestone.uk/hum3/lofigui/examples/06_notes_crud",
		},
		{
			name:       "08_water_tank_multi",
			path:       "examples/08_water_tank_multi/go",
			moduleName: "git.bytestone.uk/hum3/lofigui/examples/08_water_tank_multi",
		},
	}
	...
	for _, ex := range examples {
		t.Run(ex.name, func(t *testing.T) {
			...
			if !strings.Contains(modContent, "git.bytestone.uk/hum3/lofigui") {
				t.Errorf("go.mod does not require lofigui package")
			}

			if !strings.Contains(modContent, "replace git.bytestone.uk/hum3/lofigui => ../../..") {
...
func testGoExampleHTTPHandlers(t *testing.T) {
	...
	for _, exPath := range examples {
		t.Run(exPath, func(t *testing.T) {
			...
			if !strings.Contains(string(output), "git.bytestone.uk/hum3/lofigui") {
...
func testGoExampleStructure(t *testing.T) {
	examples := []struct {
		...
		{
			// 01 uses the compact API, so it has no go/templates/ — the
			// library auto-generates the SW bootstrap at deploy time.
			name: "01_hello_world",
		...
		{
			name: "01a_hello_world_explicit",
			path: "examples/01a_hello_world_explicit",
			requiredFiles: []string{
				"go/main.go",
				"go/go.mod",
				"go/routes.go",
				"go/templates/hello.html",
				"go/templates/sw/index.html",
				"go/templates/sw/sw.js",
				"go/templates/sw/wasmhttp_sw.js",
			},
		},
		{
			name: "01b_hello_world_explicit_gzip",
			path: "examples/01b_hello_world_explicit_gzip",
			requiredFiles: []string{
				"go/main.go",
				"go/go.mod",
				"go/routes.go",
		...
		{
			// 02 uses the compact API (no go/templates/) like 01.
			name: "02_svg_graph",
